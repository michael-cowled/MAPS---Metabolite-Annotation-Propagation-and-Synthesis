...
dataset.id <- "HGMD_0104"

metadata <- read.csv("Y:/MA_BPA_Microbiome/LCMS data/HGMD_0104_MC1-97/metadata.csv")

###---User-defined metadata categories---###

group.1 <- filter(metadata, ATTRIBUTE_Group == "CR")
group.2 <- filter(metadata, ATTRIBUTE_Group == "FR")
name.group.1 <- "Crude Fraction"
name.group.2 <- "Subfraction"
...
final.annotation.df4 <- filter(final.annotation.df, !is.na(compound.name))
...
datasets$confidence.level <- as.numeric(as.character(datasets$confidence.level))
...
# Count features per NPC.superclass and dataset.id

# Read the filtered dataset and clean invalid superclasses
filtered_datasets <- final_result %>%
  filter(!(c %in% c("None", "Others", "N/A", "NA", "")) & !is.na(NPC.superclass))

plot_data <- filtered_datasets %>%
  group_by(NPC.superclass, dataset.id) %>%
  summarise(Feature_Count = n(), .groups = "drop")

# Create and save the plot in one go     ###UNIQUE
ggplot(plot_data, aes(x = Feature_Count, y = NPC.superclass, fill = dataset.id)) +
