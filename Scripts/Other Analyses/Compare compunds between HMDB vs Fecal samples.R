...
write.csv(merged_df_fecal, "merged_df_fecal.csv", row.names = FALSE, na = "")
write.csv(merged_df_HMDB, "merged_df_HMDB.csv", row.names = FALSE, na = "")
...
write.csv(df_HMDB_no_cid, "HMDB_No_CID.csv", row.names = FALSE, na = "")
write.csv(df_feces_no_cid, "Fecal_No_CID.csv", row.names = FALSE, na = "")


# Ensure relevant columns are character
df_feces_no_cid$Best.Annotation.Smiles <- as.character(df_feces_no_cid$Best.Annotation.Smiles)
df_HMDB_no_cid$SMILES <- as.character(df_HMDB_no_cid$SMILES)
df_feces_no_cid$Best.Annotation <- as.character(df_feces_no_cid$Best.Annotation)
df_HMDB_no_cid$name <- as.character(df_HMDB_no_cid$name)

# Step 1: Match by SMILES
matched_smiles <- intersect(df_feces_no_cid$Best.Annotation.Smiles, df_HMDB_no_cid$SMILES)
matched_by_smiles <- matched_smiles[!is.na(matched_smiles) & matched_smiles != ""]

# Step 2: Match by annotation name (name vs Best.Annotation)
matched_names <- intersect(df_feces_no_cid$Best.Annotation, df_HMDB_no_cid$name)
...
# In fecal, filter if match in Best.Annotation.Smiles or Best.Annotation
no_cid_namesmiles_match_fecal <- df_feces_no_cid %>%
  filter(Best.Annotation.Smiles %in% no_cid_match_name_smiles |
           Best.Annotation %in% no_cid_match_name_smiles)
...
write.csv(no_cid_namesmiles_match_fecal, "no_cid_namesmiles_match_fecal.csv", row.names = FALSE, na = "")
write.csv(no_cid_namesmiles_match_HMDB, "no_cid_namesmiles_match_HMDB.csv", row.names = FALSE, na = "")
...
write.csv(df2_overlap, "HMDB_Overlapping_CIDs.csv", row.names = FALSE, na = "")
