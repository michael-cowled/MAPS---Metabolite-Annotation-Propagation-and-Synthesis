#Making Proportion of pathways per apple figure                                   ####EDITED FOR Pathway rather than pathway
...
class.data <- read.csv("ms2query.csv") %>%                            #####!!!###### Check spelling of filename
  filter(ms2query_model_prediction >= 0.6)
names(class.data)[1] <- "row.ID"
class.data <- select(class.data, "row.ID", "npc_pathway_results")
...
class.data <- class.data %>%
  mutate(npc_pathway_results = ifelse(npc_pathway_results == "", "Unclassified", npc_pathway_results))
...
#Function: SUMMARISE_pathways:
summarise_pathways <- function(j, merged_data) {
  pathways <- unique(merged_data$npc_pathway_results)
  temp_data_frame <- data.frame()  # Initialize an empty dataframe to store results
  
  # Iterate through each unique value in pathways
  for (i in pathways) {
    print(i)
    # Filter merged_data based on npc_pathway_results == i
    filt_data <- merged_data[merged_data$npc_pathway_results == i, c(1, j)]
    ...
    temp_df <- data.frame(npc_pathway_results = i, total_sum = total_sum)
    
    # Rename colnames
    colnames(temp_df) <- c("npc_pathway_results", colnames(merged_data)[j])
...
merged_df <- summarise_pathways(j = 2, merged_data)

# Loop over columns 3 to ncol(merged_data)
for (j in 3:ncol(merged_data)) {
  # Call the summarise_pathways function for each column
  tdf <- summarise_pathways(j, merged_data)
  
  # Merge the current dataframe with the merged dataframe
  merged_df <- merge(merged_df, tdf, by = "npc_pathway_results", all = TRUE)
}
...
long <- merged_df_perc %>% 
  pivot_longer(
    cols = -npc_pathway_results,  # Exclude the 'npc_pathway_results' column
    names_to = "Sample",
    values_to = "Perc"
  )
...
category_colors <- rainbow(length(unique(long$'npc_pathway_results')))
...
plot_obj <- ggplot(filtered_long, aes(x = Sample, y = npc_pathway_results, size = Perc, color = npc_pathway_results)) +
  geom_point() +
  scale_size(range = c(1, 8)) +
  scale_color_manual(values = category_colors) +
  labs(x = "Filename", y = "pathways", size = "Proportion of Metabolites") +
  guides(color = FALSE) +
  theme_minimal() +
  theme(panel.border = element_rect(color = "black", fill = NA, size = 1),
        axis.text.x = element_text(angle = 90, hjust = 1)) +  # Rotate x-axis labels
  scale_y_discrete(limits = rev(levels(filtered_long$npc_pathway_results)))
