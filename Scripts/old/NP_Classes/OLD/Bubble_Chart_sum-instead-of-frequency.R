#Making Proportion of superclasss per apple figure                                   ####EDITED for Peak Area rather than Number based
library(ggplot2)
library(reshape2) 
library(dplyr)
library(tidyr)
...
quant.data[, 2:ncol(quant.data)][quant.data[, 2:ncol(quant.data)] < 10000] <- 0   ###NOISE THRESHOLD
...
merged_data <- select(QUERY_TABLE, -row.ID) %>%   
  select(-Midrun_blank_01, -Midrun_blank_02, -Prerun_blank_01)                  ##NOw remove blanks
