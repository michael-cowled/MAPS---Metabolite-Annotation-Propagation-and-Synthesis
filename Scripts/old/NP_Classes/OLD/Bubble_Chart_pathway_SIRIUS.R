#Making Proportion of superclass per apple figure                                   ####LOOKING INTO SIRIUS _ CANOPUS RESULTS
library(ggplot2)
library(reshape2) 
library(dplyr)
library(tidyr)
...
class.data <-readr::read_tsv("canopus_compound_summary.tsv")                      #####!!!###### Check spelling of filename
names(class.data)[c(5,6,22)] <- c("npc_pathway_results", "prob", "row.ID")
class.data <- filter(class.data, prob >= 0.9)
...
summarise_superclasss <- function(j, merged_data) {
  superclasss <- unique(merged_data$npc_pathway_results)
...
plot_obj <- ggplot(filtered_long, aes(x = Sample, y = npc_pathway_results, size = Perc, color = npc_pathway_results)) +
  ...
  labs(x = "Filename", y = "superclasss", size = "Proportion of Metabolites") +
  guides(color = FALSE) +
  theme_minimal() +
